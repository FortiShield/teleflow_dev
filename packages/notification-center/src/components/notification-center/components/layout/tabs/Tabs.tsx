...
import { useTeleflowTheme } from '../../../../../hooks';
...
export function Tabs({ children, onTabChange, value }: ITabsProps) {
  const { theme: teleflowTheme, common, colorScheme } = useTeleflowTheme();
  const { classes } = useTabStyles({ teleflowTheme, common, colorScheme });
