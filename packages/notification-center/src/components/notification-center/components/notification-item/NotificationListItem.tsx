...
import { useTeleflowTheme, useNotificationCenter, useTranslations, useNotifications } from '../../../../hooks';
...
export function NotificationListItem({
  ...
  const { theme: teleflowTheme, colorScheme } = useTeleflowTheme();
  ...
  const { classes } = useDropdownStyles({ teleflowTheme });
  ...
  return (
    <div
      className={cx(
        ...
        listItemClassName,
        unread ? unreadNotificationStyles(teleflowTheme) : readNotificationStyles(teleflowTheme),
          ...
          >
            ...
            <div
              className={cx(
                ...
                timeMarkClassName(teleflowTheme, unread),
...
const SettingsActionWrapper = styled.div<{ teleflowTheme: ITeleflowTheme }>`
  color: ${({ teleflowTheme }) => teleflowTheme.layout?.wrapper.secondaryFontColor};
`;

const unreadNotificationStyles = (teleflowTheme: ITeleflowTheme) => css`
  background: ${teleflowTheme?.notificationItem?.unread?.background};
  box-shadow: ${teleflowTheme?.notificationItem?.unread?.boxShadow};
  color: ${teleflowTheme?.notificationItem?.unread?.fontColor};
  font-weight: 700;
  ...
  &:before {
    ...
    background: ${teleflowTheme?.notificationItem?.unread?.notificationItemBeforeBrandColor};
  }
`;

const readNotificationStyles = (teleflowTheme: ITeleflowTheme) => css`
  color: ${teleflowTheme?.notificationItem?.read?.fontColor};
  background: ${teleflowTheme?.notificationItem?.read?.background};
  font-weight: 400;
  font-size: 14px;
`;
...
const timeMarkClassName = (teleflowTheme: ITeleflowTheme, unread?: boolean) => css`
  min-width: 55px;
  font-size: 12px;
  font-weight: 400;
  opacity: 0.5;
  line-height: 14.4px;
  color: ${unread
    ? teleflowTheme?.notificationItem?.unread?.timeMarkFontColor
    : teleflowTheme?.notificationItem?.read?.timeMarkFontColor};
`;
...
const dotsClassName = (theme: ITeleflowTheme) => css`
  color: ${theme?.actionsMenu?.dotsButtonColor};
`;

const useDropdownStyles = createStyles((theme: MantineTheme, { teleflowTheme }: { teleflowTheme: ITeleflowTheme }) => {
  return {
    dropdown: {
      backgroundColor: teleflowTheme.actionsMenu?.dropdownColor,
      border: 'none',
    },
    item: {
      borerRadius: '5px',
      color: teleflowTheme.actionsMenu?.fontColor,
      fontWeight: 400,
      fontSize: '14px',
      '&:hover': {
        background: teleflowTheme.actionsMenu?.hoverColor,
      },
    },
    arrow: {
      backgroundColor: teleflowTheme.actionsMenu?.dropdownColor,
      borderColor: teleflowTheme.actionsMenu?.dropdownColor,
    },
