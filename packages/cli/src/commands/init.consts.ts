...
export const introQuestions: ListQuestionOptions[] = [
  {
    ...
    validate(input: any, answers?): boolean | string | Promise<boolean | string> {
      if (
        ['test', 'testteleflow', 'testapp', 'teleflowtest'].includes(
          input
            ?.trim()
            .toLowerCase()
            .replace(/[^\w\s]/gi, '')
...
We recommend writing your project or company name ;) If you're just curious about Teleflow, type 'curious'
        `;
      } else if (!input?.trim().length) {
        return `Please enter a valid name. If you're just curious about Teleflow, type 'curious' ;)`;
      }
