...
const teleflowIntegration: IntegrationEntity = {
  _environmentId: 'env-test-123',
  _id: 'integration-test-teleflow-123',
  _organizationId: 'org-test-123',
  active: true,
  channel: ChannelTypeEnum.EMAIL,
  credentials: {},
...
jest.mock('../get-decrypted-integrations', () => ({
  ...
  GetDecryptedIntegrations: jest.fn(() => ({
    execute: jest.fn(() => teleflowIntegration),
  })),
}));
...
describe('select integration', function () {
  ...
  it('should return the teleflow integration', async function () {
    ...
    expect(integration).not.toBeNull();
    expect(integration?.providerId).toEqual(EmailProviderIdEnum.Teleflow);
  });
