...
import { Teleflow, defaultRetryCondition } from '../index';
...
describe('Teleflow Node.js package - Retries and idempotency-key', () => {
  afterEach(() => {
    nock.cleanAll();
    nock.enableNetConnect();
  });

  const teleflow = new Teleflow('fake-key', {
  ...
  it('should retry trigger and generate idempotency-key only once for request', async () => {
    ...
    const result = await teleflow.trigger('fake-workflow', {
      to: { subscriberId: '123' },
      payload: {},
    });
  ...
  it('should retry getting topics list', async () => {
    ...
    const result = await teleflow.topics.list({});
  ...
  it('should fail after reaching max retries', async () => {
    ...
    await expect(teleflow.topics.list({})).rejects.toMatchObject({
      response: { status: 500 },
    });
  });
  ...
  it('should retry on various errors until it reach successful response', async () => {
    ...
    const teleflowClient = new Teleflow('fake-key', {
      backendUrl: BACKEND_URL,
      retryConfig: {
        initialDelay: 0,
        waitMin: 0.2,
    ...
    const result = await teleflowClient.topics.list({});
    expect(result.status).toEqual(200);
  });
