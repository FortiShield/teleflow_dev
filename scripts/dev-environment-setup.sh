...
# PNPM is the package manager used in Teleflow's monorepo
install_pnpm () {
    PNPM_VERSION="8.9.0"
...
clone_monorepo () {
    ...
    if [[ -z "$SKIP" ]]; then
        echo ""
        echo "❓ Do you want to clone Teleflow's monorepo? ($POSITIVE_RESPONSE / $NEGATIVE_RESPONSE)"
        read -p " > " RESPONSE
	echo ""

    	if [[ "$RESPONSE" == "$POSITIVE_RESPONSE" ]]; then
            REPOSITORY="git@github.com:khulnasoft/teleflow.git"
            DESTINATION_FOLDER="$HOME/Dev"
            TELEFLOW_FOLDER="$DESTINATION_FOLDER/teleflow"

            [[ ! -d "$DESTINATION_FOLDER" ]] && mkdir "$DESTINATION_FOLDER"
            if [[ ! -d "$TELEFLOW_FOLDER" ]]; then
                git clone "$REPOSITORY $TELEFLOW_FOLDER"
	        success_message "Teleflow monorepo"
            else
                already_installed_message "Teleflow monorepo"
            fi
        fi
    else
        skip_message "Teleflow monorepo"
        echo "$SKIP"
    fi
}

# Teleflow set of tools chosen
install_teleflow_tools () {
    check_git
    make_zsh_default_shell
    install_ohmyzsh
