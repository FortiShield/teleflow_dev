      ...
      case ChangeEntityTypeEnum.TRANSLATION_GROUP:
        try {
          if (process.env.TELEFLOW_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') {
            if (!require('@teleflow/ee-translation')?.PromoteTranslationGroupChange) {
              throw new BadRequestException('Translation module is not loaded');
            }
            const usecase = this.moduleRef.get(require('@teleflow/ee-translation')?.PromoteTranslationGroupChange, {
              strict: false,
            });
