...
import { AnalyticsService, CreateChange, CreateChangeCommand, PlatformException } from '@teleflow/application-generic';
  ...
  async execute(usecaseCommand: CreateNotificationTemplateCommand) {
    ...
    try {
      if (process.env.TELEFLOW_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') {
        if (!require('@teleflow/ee-translation')?.TranslationsService) {
          throw new PlatformException('Translation module is not loaded');
        }
        const service = this.moduleRef.get(require('@teleflow/ee-translation')?.TranslationsService, { strict: false });
