...
describe(`Trigger event - ${eventTriggerPath} (POST)`, function () {
  ...
  describe(`Trigger Event - ${eventTriggerPath} (POST)`, function () {
    ...
    it('should store jobs & message provider id successfully', async function () {
      ...
      expect(inAppMessage?.providerId).to.equal(InAppProviderIdEnum.Teleflow);
      ...
      expect(inAppJob?.providerId).to.equal(InAppProviderIdEnum.Teleflow);
    ...
    it('should use Teleflow integration for new orgs', async function () {
      ...
      expect(message!.providerId).to.equal(EmailProviderIdEnum.Teleflow);
    });
...
export async function sendTrigger(
  ...
  return await axiosInstance.post(
    ...
    {
      name: template.triggers[0].identifier,
      to: [{ subscriberId: newSubscriberIdInAppNotification, lastName: 'Smith', email: 'test@email.teleflow' }],
      payload: {
        organizationName: 'Umbrella Corp',
