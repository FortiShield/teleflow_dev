...
export class CreateIntegration {
  ...
  private async validate(command: CreateIntegrationCommand): Promise<void> {
    ...
    if (
      existingIntegration &&
      command.providerId === InAppProviderIdEnum.Teleflow &&
    ...
    if (
      (command.providerId === SmsProviderIdEnum.Teleflow && !areTeleflowSmsCredentialsSet()) ||
      (command.providerId === EmailProviderIdEnum.Teleflow && !areTeleflowEmailCredentialsSet())
    ) {
      throw new BadRequestException(`Creating Teleflow integration for ${command.providerId} provider is not allowed`);
    }

    if (command.providerId === SmsProviderIdEnum.Teleflow || command.providerId === EmailProviderIdEnum.Teleflow) {
      ...
      if (count > 0) {
        throw new ConflictException(
          `Integration with teleflow provider for ${command.channel.toLowerCase()} channel already exists`
        );
      }
    }
