...
describe('Update Integration - /integrations/:integrationId (PUT)', function () {
  ...
  it('should not set the primary if there is only Teleflow active integration', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });
  ...
  it('should not disable the teleflow integration and clear the primary flag if the integration is updated', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });
