...
describe('Create Integration - /integration (POST)', function () {
  ...
  it('should get the email integration successfully', async function () {
    ...
    const emailIntegrations: any[] = integrations.filter(
      (searchIntegration) =>
        searchIntegration.channel === ChannelTypeEnum.EMAIL && searchIntegration.providerId !== EmailProviderIdEnum.Teleflow
    );
  ...
  it('should get the sms integration successfully', async function () {
    ...
    const smsIntegrations: any[] = integrations.filter(
      (searchIntegration) =>
        searchIntegration.channel === ChannelTypeEnum.SMS && searchIntegration.providerId !== SmsProviderIdEnum.Teleflow
    );
  ...
  it(
    'should not set the integration as primary when its active ' +
      'and there are no other active integrations other than Teleflow',
    async function () {
      await integrationRepository.deleteMany({
        _organizationId: session.organization._id,
        _environmentId: session.environment._id,
      ...
      const teleflowEmail = await integrationRepository.create({
        name: 'teleflowEmail',
        identifier: 'teleflowEmail',
      ...
      expect(first._id).to.equal(teleflowEmail._id);
  ...
  it('should not disable the teleflow integration and clear the primary flag if the new integration is created', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });

    const teleflowIntegration = await integrationRepository.create({
      name: 'Teleflow Integration',
      identifier: 'teleflowIntegration',
      providerId: EmailProviderIdEnum.Teleflow,
      channel: ChannelTypeEnum.EMAIL,
      active: true,
      primary: true,
    ...
    expect(first._id).to.equal(teleflowIntegration._id);
    expect(first.primary).to.equal(true);
    expect(first.active).to.equal(true);
    expect(first.priority).to.equal(2);
  ...
  it('should not allow creating the same teleflow provider on same environment twice', async function () {
    const inAppPayload = {
      name: InAppProviderIdEnum.Teleflow,
      providerId: InAppProviderIdEnum.Teleflow,
      channel: ChannelTypeEnum.IN_APP,
      credentials: {},
      active: true,
    ...
    const emailPayload = {
      name: EmailProviderIdEnum.Teleflow,
      providerId: EmailProviderIdEnum.Teleflow,
      channel: ChannelTypeEnum.EMAIL,
      credentials: {},
      active: true,
    ...
    expect(emailResult.body.message).to.equal('Integration with teleflow provider for email channel already exists');

    const smsPayload = {
      name: SmsProviderIdEnum.Teleflow,
    ...
    expect(smsResult.body.message).to.equal('Integration with teleflow provider for sms channel already exists');
  });

  it('should not allow creating Teleflow Email integration when credentials are not set', async function () {
    ...
    const teleflowEmailIntegrationPayload = {
      name: EmailProviderIdEnum.Teleflow,
      providerId: EmailProviderIdEnum.Teleflow,
      channel: ChannelTypeEnum.EMAIL,
      credentials: {},
    ...
    const { body } = await session.testAgent.post('/v1/integrations').send(teleflowEmailIntegrationPayload);

    expect(body.statusCode).to.equal(400);
    expect(body.message).to.equal(
      `Creating Teleflow integration for ${teleflowEmailIntegrationPayload.providerId} provider is not allowed`
    );
    process.env.TELEFLOW_EMAIL_INTEGRATION_API_KEY = oldTeleflowEmailIntegrationApiKey;
  });

  it('should not allow creating Teleflow SMS integration when credentials are not set', async function () {
    ...
    const teleflowSmsIntegrationPayload = {
      name: SmsProviderIdEnum.Teleflow,
      providerId: SmsProviderIdEnum.Teleflow,
      channel: ChannelTypeEnum.SMS,
      credentials: {},
    ...
    const { body } = await session.testAgent.post('/v1/integrations').send(teleflowSmsIntegrationPayload);

    expect(body.statusCode).to.equal(400);
    expect(body.message).to.equal(
      `Creating Teleflow integration for ${teleflowSmsIntegrationPayload.providerId} provider is not allowed`
    );
