...
import { TracingModule } from '@teleflow/application-generic';
...
const enterpriseImports = (): Array<Type | DynamicModule | Promise<DynamicModule> | ForwardReference> => {
  ...
  try {
    if (process.env.TELEFLOW_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') {
      if (require('@teleflow/ee-auth')?.EEAuthModule) {
        modules.push(require('@teleflow/ee-auth')?.EEAuthModule);
      }
      if (require('@teleflow/ee-translation')?.EnterpriseTranslationModule) {
        modules.push(require('@teleflow/ee-translation')?.EnterpriseTranslationModule);
      }
      if (require('@teleflow/ee-billing')?.BillingModule) {
        modules.push(require('@teleflow/ee-billing')?.BillingModule.forRoot());
      }
    }
  } catch (e) {
