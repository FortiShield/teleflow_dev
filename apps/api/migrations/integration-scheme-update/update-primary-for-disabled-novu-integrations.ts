...
import { EmailProviderIdEnum, SmsProviderIdEnum } from '@teleflow/shared';

import { AppModule } from '../../src/app.module';

export async function run() {
  console.log('Update the primary and priority fields for inactive Teleflow integrations\n');
  ...
  for (const environment of environments) {
    ...
    const integrations = await integrationRepository.find(
      {
        ...
        primary: true,
        providerId: {
          $in: [EmailProviderIdEnum.Teleflow, SmsProviderIdEnum.Teleflow],
        },
      },
      undefined,
      { sort: { createdAt: -1 } }
    ...
    console.log(
      `Found ${integrations.length} inactive and primary Teleflow integrations ` +
    ...
    if (ids.length > 0) {
      console.log(`Updating Teleflow integrations with: `, { primary: false, priority: 0 });
