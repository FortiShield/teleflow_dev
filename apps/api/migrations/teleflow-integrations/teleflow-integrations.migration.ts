...
const createTeleflowIntegration = async (
  ...
  const providerId = channel === ChannelTypeEnum.SMS ? SmsProviderIdEnum.Teleflow : EmailProviderIdEnum.Teleflow;
  const name = channel === ChannelTypeEnum.SMS ? 'Teleflow SMS' : 'Teleflow Email';
...
export async function createTeleflowIntegrations() {
  ...
  console.log('start migration - teleflow integrations');
