...
import { ChannelTypeEnum, EmailBlockTypeEnum } from '@teleflow/shared';
...
describe('Compile E-mail Template', function () {
  ...
  it('should apply subject variable if provided', async function () {
    const subjectText = 'Teleflow Test';
    const { html, subject } = await useCase.execute(
      CompileEmailTemplateCommand.create({
        organizationId: session.organization._id,
        environmentId: session.environment._id,
  ...
  it('should apply sender name variable if provided', async function () {
    const senderNameTest = 'Teleflow Test';
  ...
  describe('Backwards compatibility', function () {
    ...
    it('should add default teleflow layout for visual editor templates', async function () {
      const { html, subject } = await useCase.execute(
        CompileEmailTemplateCommand.create({
          organizationId: session.organization._id,
          environmentId: session.environment._id,
