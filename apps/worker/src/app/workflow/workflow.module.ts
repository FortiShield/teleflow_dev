...
import { JobTopicNameEnum } from '@teleflow/shared';
...
const enterpriseImports = (): Array<Type | DynamicModule | Promise<DynamicModule> | ForwardReference> => {
  ...
  try {
    if (process.env.TELEFLOW_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') {
      ...
      if (require('@teleflow/ee-translation')?.EnterpriseTranslationModuleWithoutControllers) {
        Logger.log('Importing enterprise translations module', 'EnterpriseImport');
        modules.push(require('@teleflow/ee-translation')?.EnterpriseTranslationModuleWithoutControllers);
      }
      if (require('@teleflow/ee-billing')?.BillingModule) {
        ...
        modules.push(require('@teleflow/ee-billing')?.BillingModule.forRoot(activeWorkers));
      }
    }
  } catch (e) {
