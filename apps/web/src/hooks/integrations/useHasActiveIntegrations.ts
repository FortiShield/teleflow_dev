  ...
  const hasActiveIntegration = useMemo(() => {
    ...
    if (isActive && isEmailStep) {
      const isTeleflowProvider = activeIntegrationsByEnv?.some(
        (integration) => integration.providerId === EmailProviderIdEnum.Teleflow && integration.primary
      );
    ...
    if (isActive && isSmsStep) {
      const isTeleflowProvider = activeIntegrationsByEnv?.some(
        (integration) => integration.providerId === SmsProviderIdEnum.Teleflow && integration.primary
      );
