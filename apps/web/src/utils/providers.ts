...
if (IS_DOCKER_HOSTED) {
  initialEmailProviders = emailProviders.filter((provider) => provider.id !== EmailProviderIdEnum.Teleflow);
  initialSmsProviders = smsProviders.filter((provider) => provider.id !== SmsProviderIdEnum.Teleflow);
}
