...
import { ChannelTypeEnum, ICreateIntegrationBodyDto, TELEFLOW_PROVIDERS, providers } from '@teleflow/shared';
import {
  ActionButton,
  Button,
  colors,
...
export function CreateProviderInstanceSidebar({
  ...
  const showTeleflowProvidersErrorMessage = useMemo(() => {
    if (!provider || integrations.length === 0 || !TELEFLOW_PROVIDERS.includes(provider.id)) {
      return false;
    }
