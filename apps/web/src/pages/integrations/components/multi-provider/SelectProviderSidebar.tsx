...
import { ChannelTypeEnum, InAppProviderIdEnum } from '@teleflow/shared';
import {
  colors,
  Sidebar,
  Button,
...
export function SelectProviderSidebar({
  ...
  const inAppCount: number = useMemo(() => {
    const count = integrations.filter(
      (integration) =>
        integration.channel === ChannelTypeEnum.IN_APP && integration.providerId === InAppProviderIdEnum.Teleflow
    ).length;
