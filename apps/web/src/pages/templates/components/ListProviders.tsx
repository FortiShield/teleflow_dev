...
import { ChannelTypeEnum, TELEFLOW_SMS_EMAIL_PROVIDERS } from '@teleflow/shared';

import { When } from '../../../components/utils/When';
import { Button, colors, Tooltip } from '@teleflow/design-system';
...
const LackIntegrationByType = ({
  ...
  const { environment: currentEnvironment } = useEnvController();
  const containsTeleflowProvider = TELEFLOW_SMS_EMAIL_PROVIDERS.some(
      ...
      <When
        truthy={
          providers.filter((provider) => provider.connected && provider.environmentId === currentEnvironment?._id)
            .length === 1 && containsTeleflowProvider
        }
      >
        <div
          style={{
