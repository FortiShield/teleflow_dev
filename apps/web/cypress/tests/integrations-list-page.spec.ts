...
describe('Integrations List Page', function () {
  ...
  it('should show the select provider sidebar', () => {
    cy.task('deleteProvider', {
      ...
      emailProviders
        .filter((provider) => provider.id !== EmailProviderIdEnum.Teleflow)
        .forEach((provider) => {
      ...
      smsProviders
        .filter((provider) => provider.id !== SmsProviderIdEnum.Teleflow)
        .forEach((provider) => {
  ...
  it('should show the Teleflow in-app integration', () => {
    cy.intercept('GET', '*/integrations', async () => {
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }).as('getIntegrations');
    cy.intercept('POST', '*/integrations', async () => {
    ...
    clickOnListRow('Teleflow In-App');
    ...
    cy.window().then((win) => {
      if (win.isDarkTheme) {
        cy.getByTestId(`selected-provider-image-${InAppProviderIdEnum.Teleflow}`).should(
          'have.attr',
          'src',
          `/static/images/providers/dark/square/${InAppProviderIdEnum.Teleflow}.svg`
        );
        return;
      }
      ...
      cy.getByTestId(`selected-provider-image-${InAppProviderIdEnum.Teleflow}`).should(
        'have.attr',
        'src',
        `/static/images/providers/light/square/${InAppProviderIdEnum.Teleflow}.svg`
      );
    });

    cy.getByTestId('provider-instance-name').should('be.visible').should('have.value', 'Teleflow In-App');
    cy.getByTestId('provider-instance-identifier').should('contain.value', 'teleflow-in-app');
    cy.getByTestId('hmac').should('not.be.checked');
    cy.getByTestId('teleflow-in-app-frameworks').contains('Integrate In-App using a framework below');
    cy.getByTestId('teleflow-in-app-frameworks').contains('React');
    cy.getByTestId('teleflow-in-app-frameworks').contains('Angular');
    cy.getByTestId('teleflow-in-app-frameworks').contains('Web Component');
    cy.getByTestId('teleflow-in-app-frameworks').contains('Headless');
    cy.getByTestId('teleflow-in-app-frameworks').contains('Vue');
    cy.getByTestId('teleflow-in-app-frameworks').contains('iFrame');
    cy.getByTestId('update-provider-sidebar-update').should('be.disabled').contains('Update');
  });

  it('should show the Teleflow in-app integration - React guide', () => {
    cy.intercept('GET', '*/integrations', async () => {
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }).as('getIntegrations');
    cy.intercept('POST', '*/integrations', async () => {
    ...
    cy.getByTestId('update-provider-sidebar').should('be.visible');
    cy.getByTestId('teleflow-in-app-frameworks').contains('React').click();
  ...
  it('should show the Teleflow Email integration sidebar', () => {
    ...
    clickOnListRow('Teleflow Email');

    cy.wait('@getTeleflowEmailLimit');
    cy.getByTestId('update-provider-sidebar-teleflow').should('be.visible');
    cy.window().then((win) => {
      if (win.isDarkTheme) {
        cy.getByTestId('update-provider-sidebar-teleflow').find(
    ...
    cy.getByTestId('provider-instance-channel').should('contain', 'Email');
    cy.getByTestId('provider-instance-environment').should('contain', 'Development');
    cy.getByTestId('update-provider-sidebar-teleflow')
      .getByTestId('provider-instance-name')
      .should('have.value', 'Teleflow Email');
    cy.getByTestId('update-provider-sidebar-teleflow').contains('Test Provider');
    cy.getByTestId('teleflow-provider-limits').then((el) => {
      expect(el.get(0).innerText).to.eq(
        'Teleflow provider allows sending max 300 emails per month,\nto send more messages, configure a different provider'
      );
    });
  ...
  it('should show the Teleflow SMS integration sidebar', () => {
    cy.intercept('GET', '*/integrations', async (req) => {
      req.continue((res) => {
        const {
          body: { data },
        ...
        res.body.data = [
          ...data,
          {
            _id: SmsProviderIdEnum.Teleflow,
            _environmentId: firstIntegration._environmentId,
            providerId: SmsProviderIdEnum.Teleflow,
            active: true,
            channel: ChannelTypeEnum.SMS,
            name: 'Teleflow SMS',
            identifier: SmsProviderIdEnum.Teleflow,
          },
        ];
      });
    }).as('getIntegrations');
    ...
    clickOnListRow('Teleflow SMS');

    cy.wait('@getTeleflowSmsLimit');
    cy.getByTestId('update-provider-sidebar-teleflow').should('be.visible');
    cy.window().then((win) => {
      if (win.isDarkTheme) {
        cy.getByTestId('update-provider-sidebar-teleflow').find(
    ...
    cy.getByTestId('update-provider-sidebar-teleflow')
      .getByTestId('provider-instance-name')
      .should('have.value', 'Teleflow SMS');
    cy.getByTestId('update-provider-sidebar-teleflow').contains('Test Provider');
    cy.getByTestId('teleflow-provider-limits').then((el) => {
      expect(el.get(0).innerText).to.eq(
        'Teleflow provider allows sending max 20 messages per month,\nto send more messages, configure a different provider'
      );
    });
    cy.getByTestId('limitbar-limit').then((el) => {
  ...
  it('should not allow creating a teleflow provider for the same environment if it already exists', () => {
    cy.intercept('*/integrations', async () => {
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }).as('getIntegrations');
    cy.intercept('*/environments').as('getEnvironments');
    ...
    cy.getByTestId(`provider-${EmailProviderIdEnum.Teleflow}`).contains('Teleflow').click();

    cy.window().then((win) => {
      if (win.isDarkTheme) {
        cy.getByTestId(`selected-provider-image-${EmailProviderIdEnum.Teleflow}`).should(
          'have.attr',
          'src',
          `/static/images/providers/dark/square/${EmailProviderIdEnum.Teleflow}.svg`
        );
        return;
      }
      ...
      cy.getByTestId(`selected-provider-image-${EmailProviderIdEnum.Teleflow}`).should(
        'have.attr',
        'src',
        `/static/images/providers/light/square/${EmailProviderIdEnum.Teleflow}.svg`
      );
    });
    cy.getByTestId('selected-provider-name').should('be.visible').contains('Teleflow');

    cy.getByTestId('select-provider-sidebar-next').should('not.be.disabled').contains('Next').click();
    cy.getByTestId('teleflow-provider-error').contains('You can only create one Teleflow Email per environment.');
    cy.getByTestId('create-provider-instance-sidebar-create').should('be.disabled');
  });
