...
import type { ITeleflowThemeProvider, INotificationCenterStyles } from '@teleflow/notification-center';
import { IMessage, IOrganizationEntity, ButtonTypeEnum, isBrowser } from '@teleflow/shared';
...
export function NotificationCenterWidget(props: INotificationCenterWidgetProps) {
  ...
  const [theme, setTheme] = useState<ITeleflowThemeProvider>({});
    ...
    <>
      ...
      {frameInitialized && (
        <TeleflowProvider
          ...
          styles={styles}
        >
          <TeleflowNotificationCenterWrapper doLogout={doLogout} setDoLogout={setDoLogout}>
            <NotificationCenter
            ...
            />
          </TeleflowNotificationCenterWrapper>
        </TeleflowProvider>
      )}
    </>
  );
...
function TeleflowNotificationCenterWrapper({
  children,
  doLogout,
  setDoLogout,
}: {
