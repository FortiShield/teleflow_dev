...
const mockTeleflowMessage: ISmsOptions = {
  from: 'My Company',
  to: '+33623456789',
  content: 'SMS content',
};
...
describe('sendMessage method', () => {
  ...
  test('should send message with provided option from overriding config from', async () => {
    ...
    expect(fetch.mock.calls[0][1]).toMatchObject({
      body: expect.objectToEqual('sender', mockTeleflowMessage.from),
    });
  });
  test('should send message with provided option to', async () => {
    ...
    expect(fetch.mock.calls[0][1]).toMatchObject({
      body: expect.objectToEqual('recipient', mockTeleflowMessage.to),
    });
  });
  test('should send message with provided option content', async () => {
    ...
    expect(fetch.mock.calls[0][1]).toMatchObject({
      body: expect.objectToEqual('content', mockTeleflowMessage.content),
    });
  });
  test('should return id returned in request response', async () => {
    ...
    const result = await provider.sendMessage(mockTeleflowMessage);
